...
-   **AI Financial Advisor**: Powered by Groq AI (Llama 3.3 70B Versatile with native tool-use support), offering CFO-level advice with advanced intelligence for data validation, luxury asset, live market, spending, and behavioral analysis. Features include AI personalization, memory, graceful error handling, imperative command detection, robust tool parameter handling, empathetic coaching, and **mandatory language auto-detection** (responds 100% in user's message language with zero tolerance for language mixing), with sanitized, conversational responses.
    ...
    -   **Proactive Financial Data Usage (October 28, 2025)**: ZERO TOLERANCE policy - AI now PROACTIVELY accesses user's financial profile (monthly income, monthly expenses, total savings) from the database WITHOUT asking redundant questions. System prompt displays user's complete financial data at the top with explicit instructions: "YOU ALREADY KNOW: Income $X, Expenses $Y, Savings $Z - USE THIS DATA!" AI provides personalized analysis using actual user data immediately, only asking for clarification if values are $0 or clearly missing. Maintains peak AI performance as "the best financial AI" by demonstrating awareness of user's complete financial situation.
    ...
    -   **Enhanced Tool Utilization (October 28, 2025)**: AI proactively uses **20+ specialized financial tools** covering all major personal finance needs:
        -   **Core Tools**: Transaction tracking, goal creation, budget optimization, financial estimates
        -   **Investment Tools**: Portfolio allocation, retirement planning, future value projection, luxury purchase analysis
        -   **Debt Management**: Debt payoff strategies (avalanche vs snowball), affordability analysis, financing comparisons
        -   **NEW: Essential Life Tools (October 28, 2025)**:
            -   Emergency Fund Calculator: Personalized 3-12 month recommendations based on income stability and dependents
            -   Credit Score Improvement: Step-by-step action plans with payment history, utilization, and score optimization strategies
            -   Rent Affordability: 30% rule calculator with 50/30/20 budget breakdown
            -   Mortgage Calculator: Complete PITI breakdown with lifetime cost analysis and PMI optimization
            -   Tax Optimization: Retirement account strategies, deduction maximization, and tax bracket analysis with 2024 rates
