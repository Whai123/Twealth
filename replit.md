...
- **AI Financial Advisor**: A CFO-level financial advisor ($150/hour expertise) powered by Groq AI (Llama 4 Scout) offering:
    ...
    - **Smart Confirmation Buttons**: Interactive Yes/No buttons appear when AI asks for confirmation (e.g., "Do you want me to add this to the goal?"). Mobile-responsive design with touch-friendly sizing (h-12/48px on mobile), vertical stacking on small screens, green gradient Yes button, red outlined No button, action-specific emojis, and visual feedback on selection. Buttons automatically send confirmation to AI without typing.
